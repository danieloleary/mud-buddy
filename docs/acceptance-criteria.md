...
- The app explains that the usage-file step usually takes about 3 minutes, doesn't require understanding the file, and doesn't change the user's EBMUD account.
- The page clearly says the usage file is read in the browser and isn't uploaded.
- The page clearly says Mud Buddy isn't affiliated with EBMUD and isn't an official audit, leak detector, billing tool, or EBMUD analysis.
...
- Browser report doesn't render private filename, account number, meter ID, service address, local path, raw usage file rows, or exact private identifiers.
...
- Browser upload doesn't use network requests after file selection.
- Browser upload doesn't write usage file/report data to localStorage, sessionStorage, IndexedDB, cookies, URLs, or a Mud Buddy account.
...
- EBMUD-review docs are present and don't imply affiliation, endorsement, approval, partnership, co-branding, or official status.
