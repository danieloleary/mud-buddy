...
Mud Buddy's public app is designed as a static GitHub Pages site. The primary workflow is browser-local CSV analysis.

## What happens when a user selects a CSV
...
5. The private filename, local path, account number, meter ID, service address, and raw CSV rows are not rendered.

## What should not happen

- No CSV upload to a Mud Buddy backend.
- No localStorage, sessionStorage, IndexedDB, cookie, query-string, or URL-fragment storage of the CSV.
...
Dan's real CSV test is local-only and opt-in:

```bash
MUD_BUDDY_REAL_CSV="path/to/private.csv" npm run test:local-real-csv
```

The real CSV must never be committed, packaged, uploaded, screenshotted for public launch assets, or copied into docs.
